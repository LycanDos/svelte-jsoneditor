<script lang="ts">
  ...
  const renderers = $derived(
    context.onRenderValue({
      ...
      deltaTarget: context.deltaTarget,
      deltaSources: context.deltaSources,
      deltaValueRegistry: context.deltaValueRegistry,
      deltaValueContext: context.deltaValueContext,
      selection,
      searchResultItems,
