...
<script lang="ts">
  ...
  $: rawColumnKey = path.length === 1 ? String(path[0]) : undefined
  $: useDeltaKeyUI =
    deltaMode && rawColumnKey !== undefined && deltaLanguageService.isDeltaLikeKey(rawColumnKey)
...
>
  <span class="jse-column-name">
    {#if useDeltaKeyUI && rawColumnKey}
      <DeltaKeyText keyText={rawColumnKey} languageService={deltaLanguageService} />
    {:else}
